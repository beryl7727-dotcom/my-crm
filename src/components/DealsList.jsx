...
export default function DealsList({ deals, onDealClick }) {
  if (!deals || deals.length === 0) {
    return <p className="text-sm text-slate-600">No deals found for this contact.</p>;
  }
    ...
    <div className="overflow-hidden rounded-3xl border border-slate-200">
      ...
      <div className="divide-y divide-slate-200 bg-white">
        {deals.map((deal) => (
          ...
          >
            <div>
              <p className="font-semibold text-slate-900">{deal.title || 'Untitled deal'}</p>
              <p className="text-sm text-slate-500">{deal.company || ''}</p>
            </div>
            <div className="text-slate-700">${Number(deal.amount || 0).toLocaleString()}</div>
            <div className="text-slate-700">{deal.stage || 'N/A'}</div>
            <div className="text-slate-700">{formatDate(deal.expected_close)}</div>
